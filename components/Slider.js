...
function ScrubberComponent({ values, options }) {
	const [scrubber, setScrubber] = useState(null);
	const formRef = useRef(null);

	useEffect(() => {
		const { format, initial, delay, autoplay, loop, loopDelay, alternate } =
			options || {};
		const newScrubber = Scrubber(values, {
			format,
			initial,
			delay,
			autoplay,
		...
		setScrubber(newScrubber);
		formRef.current.appendChild(newScrubber);
		return () => {
			if (newScrubber) {
				Inputs.disposal(newScrubber).then(() =>
					formRef.current.removeChild(newScrubber)
				);
			}
		};
	}, [values, options]);
